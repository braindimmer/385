...
void printArr(int arr[], int cost[], int n){
	int totalcost = 0;
    for(int i = 1; i < n; i++){
		std::cout << arr[i] << " - " << i << " -> " << cost[i] << std::endl;
        totalcost += cost[i];
    }
...
int main(){
    ...
    cout << "\n<-- Connected graph for Prims -->" << endl;
    g.Print();
    cout << "\n<-- MST of graph -->" << endl;
    prims(g);
    
    cout << "\n<-- Connected graph2 for Prims -->" << endl;
    g2.Print();
